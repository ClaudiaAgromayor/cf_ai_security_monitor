...
export interface SecurityEvent {
  id: string;
  timestamp: number;
  type: "login_attempt" | "api_call" | "data_access" | "config_change" | "unknown";
...
ACTION: [recommendation]`;
    ...
    const threatLevel = (threatMatch?.[1]?.toLowerCase() || "suspicious") as any;
    ...
    const alert: SecurityAlert = {
      ...
      action_taken: threatLevel === "critical" ? "escalated" : threatLevel === "dangerous" ? "flagged" : "none",
    };
  ...
  async logEvent(event: Omit<SecurityEvent, "id" | "timestamp">): Promise<SecurityAlert> {
  ...
  private calculateAverageThreat(): string {
    ...
    const avg =
      this.alerts.reduce((sum, alert) => sum + (threatScores[alert.threat_level] || 0), 0) / this.alerts.length;
